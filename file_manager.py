...
# Additionally, it includes functions useful to perform crop and resample
...
#===================================================================================================
#                                                                       GUIDED USER INTERFACE [Preliminary operations]
#===================================================================================================
...
def file_manager():
   ...
   # Left panel (cropping)
    ...
    ctk.CTkLabel(frame_left, text="Cropping Tool").pack(pady=5)
    
    raster1_entry = ctk.CTkEntry(frame_left, width=200, placeholder_text="Type the path here...", placeholder_text_color='grey')
    raster1_entry.pack(pady=5)
    ctk.CTkButton(frame_left, font = font_style, fg_color=button_color, text="Select Raster", command=lambda: load_file(raster1_entry)).pack(pady=5)
    
    raster2_entry = ctk.CTkEntry(frame_left, width=200, placeholder_text="Type the path here...", placeholder_text_color='grey')
    raster2_entry.pack(pady=5)
    ctk.CTkButton(frame_left, font = font_style, fg_color=button_color, text="Select Raster", command=lambda: load_file(raster2_entry)).pack(pady=5)
    
    output_dir_entry = ctk.CTkEntry(frame_left, width=200, placeholder_text="Type the path here...", placeholder_text_color='grey')
    output_dir_entry.pack(pady=5)
    ctk.CTkButton(frame_left, font = font_style, fg_color=button_color, text="Select Output Folder", command=lambda: load_directory(output_dir_entry)).pack(pady=5)
    
    ctk.CTkButton(frame_left, font = font_style_executor, fg_color=button_color, text="Crop", command=lambda: rastercutter(raster1_entry.get(), raster2_entry.get(), output_dir_entry.get())).pack(pady=10)

    # Right panel (resampling)
    frame_right = ctk.CTkFrame(window, fg_color=panel_color)
    frame_right.pack(side="right", fill="both", expand=True, padx=10, pady=10)
    ctk.CTkLabel(frame_right, text="Resampling Tool").pack(pady=5)
    
    resample_entry = ctk.CTkEntry(frame_right, width=200, placeholder_text="Type the path here...", placeholder_text_color='grey')
    resample_entry.pack(pady=5)
    ctk.CTkButton(frame_right, text="Select Raster", font=font_style, fg_color=button_color, command=lambda: load_file(resample_entry)).pack(pady=5)
    
    
    output_resample_entry = ctk.CTkEntry(frame_right, width=200, placeholder_text="Type the path here...", placeholder_text_color='grey')
    output_resample_entry.pack(pady=5)
    ctk.CTkButton(frame_right, text="Select Output Folder", font = font_style, fg_color=button_color, command=lambda: load_directory(output_resample_entry)).pack(pady=5)
    
    ctk.CTkButton(frame_right, text="Resample", font=font_style_executor, fg_color=button_color, command=lambda: resample(resample_entry.get(), resolution_entry.get(), output_resample_entry.get())).pack(pady=10)
